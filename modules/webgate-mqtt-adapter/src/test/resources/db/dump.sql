...
COPY public.devices (id, created_by, created, modified_by, modified, row_version, company, partner, product, amount, code, data, status) FROM stdin;
...
CREATE INDEX company_idx ON public.devices USING btree (company);
...
CREATE INDEX partner_idx ON public.devices USING btree (partner);
